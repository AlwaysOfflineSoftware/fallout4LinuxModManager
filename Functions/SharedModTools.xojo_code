...
Protected Module SharedModTools
	#tag Method, Flags = &h0
		Sub BackupOriginal()
		  If(App.modsFile_Plugins.Exists) Then
		    App.modsFile_Plugins.CopyTo(App.BaseDir.child("Plugins.txt.bak"))
		  End
		  
		  If(App.modsFile_UserDlContent.Exists) Then
		    App.modsFile_UserDlContent.CopyTo(App.BaseDir.child("UserDownloadedContent.txt.bak"))
		  End
		  
		  If(App.modsFile_DLC.Exists) Then
		    App.modsFile_DLC.CopyTo(App.BaseDir.child("DLCList.txt.bak"))
		  End
		  ...
		  If(App.savedSettings<>Nil) Then
		    ...
		    If(settingsContent.Count>2) Then
		      ...
		      If(Utils.ValidatePath(settingsContent(1))) Then
		        App.falloutData= New FolderItem(settingsContent(1))
		      End
	...
	#tag Method, Flags = &h0
		Sub RevertToOriginal()
		  If(App.BaseDir.child("Plugins.txt.bak").Exists) Then
		    App.modsFile_Plugins.Remove
		    App.BaseDir.child("Plugins.txt.bak").CopyTo(App.BaseDir.child("Plugins.txt"))
		  End
		  
		  If(App.BaseDir.child("UserDownloadedContent.txt.bak").Exists) Then
		    App.modsFile_UserDlContent.Remove
		    App.BaseDir.child("UserDownloadedContent.txt.bak")_
		    .CopyTo(App.BaseDir.child("UserDownloadedContent.txt"))
		  End
		  
		  If(App.BaseDir.child("DLCList.txt.bak").Exists) Then
		    App.modsFile_DLC.Remove
		    App.BaseDir.child("DLCList.txt.bak").CopyTo(App.BaseDir.child("DLCList.txt"))
		  End
	...
	#tag Method, Flags = &h0
		Sub SaveSettings()
		  ...
		  Utils.WriteFile(App.savedSettings,App.falloutData.NativePath+ EndOfLine, False)
